    ...
    public class AnnouncePanel extends JPanel {
        ...
        public AnnouncePanel(String role, int employeeId) {
            this.role = role;
            this.employeeId = employeeId; // 设置员工ID
        ...
        private void addAnnounce() {
            ...
            if (result == JOptionPane.OK_OPTION) {
                ...
                try (Connection conn = DatabaseConnection.getConnection();
                    ...
                    pstmt.setInt(4, employeeId); // 使用员工ID
                    pstmt.executeUpdate();
        ...
        private void updateAnnounce() {
            ...
            if (selectedRow >= 0) {
                ...
                int creatorId = (int) tableModel.getValueAt(selectedRow, 4); // 获取公告的创建者ID

                // 检查是否是当前用户的公告
                if (creatorId != employeeId) {
                    JOptionPane.showMessageDialog(this, "You can only update your own announcements.");
                    return;
                }
        ...
        private void deleteAnnounce() {
            ...
            if (selectedRow >= 0) {
                ...
                // 检查是否是当前用户的公告
                if (creatorId != employeeId) {
                    JOptionPane.showMessageDialog(this, "You can only delete your own announcements.");
                    return;
                }
